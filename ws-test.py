...
async def root():
    ...
    return {
        "status": "ok",
        "ws_url": WS_URL,
        "twiml_endpoint": f"https://{DOMAIN}/twiml"
    }
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            print(f"Received message: {data[:200]}...")  # Log first 200 chars
            ...
            if message["type"] == "setup":
                ...
                conversation = sessions.get(websocket.call_sid)
                if not conversation:
                    print(f"Warning: No session found for call_sid: {websocket.call_sid}")
                    conversation = [{"role": "system", "content": SYSTEM_PROMPT}]
                    sessions[websocket.call_sid] = conversation
                ...
                print(f"Unknown message type received: {message.get('type', 'unknown')}")
        ...
        print(f"Error in WebSocket: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        if call_sid:
            sessions.pop(call_sid, None)
