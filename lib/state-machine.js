...
export function isPhaseComplete(state, phase) {
  ...
  for (const field of phaseFields) {
    ...
    // Field must have an actual value (not null/undefined) AND be confirmed
    if (value === undefined || value === null || !confirmed) {
      return false;
    }
  }
...
export function getUnconfirmedFields(state) {
  ...
  for (const field of FIELD_ORDER) {
    ...
    // Only consider fields that have actual values (not null/undefined) and are unconfirmed
    if (value !== undefined && value !== null && !confirmed) {
      unconfirmed.push({
        field,
        value,
...
export function getNextFieldToCollect(state) {
  for (const field of FIELD_ORDER) {
    ...
    // Need to collect if value is missing (null or undefined)
...
export function getRemainingRequiredFields(state) {
  ...
  for (const field of REQUIRED_FIELDS) {
    ...
    // Field is remaining if it has no value (null/undefined) or is unconfirmed
    if (value === undefined || value === null || !confirmed) {
