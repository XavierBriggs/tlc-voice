...
async function handleCollectName(args, state, hestiaClient) {
  ...
  // Create or update lead in Hestia
  ...
  if (needs_confirmation || confidence < 0.8) {
    return {
      ...
      shouldRespond: true,
      responseHint: `Confirm the name by saying something like "I have your name as ${full_name}, is that correct?"`,
    };
  }
...
async function handleCollectPhone(args, state, hestiaClient) {
  ...
  return {
    ...
    shouldRespond: true,
    responseHint: 'Phone number collected. Ask if they prefer to be contacted by phone or email.',
  };
}
...
async function handleCollectEmail(args, state, hestiaClient) {
  ...
  return {
    ...
    shouldRespond: true,
    responseHint: 'Email collected. Move on to ask about the property location.',
  };
}
...
async function handleCollectPreferredContact(args, state, hestiaClient) {
  ...
  // If email preferred but no email, note that we need to collect it
  if (preferred_contact_method === 'email' && !state.collectedData.applicant.email) {
    return {
      state,
      result: { success: true, value: preferred_contact_method, needs_email: true },
      shouldRespond: true,
      responseHint: 'They prefer email. Ask for their email address.',
    };
  }
...
async function handleCheckPrequalification(state, hestiaClient) {
  ...
  if (ready) {
    ...
    return {
      state,
      result: { success: true, prequalified: true },
      shouldRespond: true,
